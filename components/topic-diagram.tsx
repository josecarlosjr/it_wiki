...
function edgePoints(from: DiagramNode, to: DiagramNode) {
  ...
  if (Math.abs(dx) >= Math.abs(dy)) {
    ...
    return {
      x1: a.x + direction * fromSize.width / 2,
      y1: a.y,
      x2: b.x - direction * toSize.width / 2,
      y2: b.y,
    };
  }
  ...
  return {
    x1: a.x,
    y1: a.y + direction * fromSize.height / 2,
    x2: b.x,
    y2: b.y - direction * toSize.height / 2,
  };
}
...
export function TopicDiagram({ spec }: DiagramProps) {
  ...
  const nodeById = Object.fromEntries(spec.nodes.map((node) => [node.id, node]));
  const markerId = `arrow-${spec.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 40)}`;

  return (
    <figure className="topic-diagram" aria-label={`Diagrama explicativo: ${spec.title}`}>
      <figcaption>{spec.title}</figcaption>
      <p className="topic-diagram-description">{spec.description}</p>
        ...
        {spec.edges.map((edge, index) => {
          ...
          return (
            <g key={`${edge.from}-${edge.to}-${index}`}>
              <line
                className={`topic-diagram-edge${edge.animated ? ' is-animated' : ''}`}
                {...points}
            ...
            <g className={`topic-diagram-node-group kind-${node.kind ?? 'control'}`} key={node.id}>
              ...
              <text className="topic-diagram-label" x={centerX} y={centerY - ((lines.length - 1) * 8)} textAnchor="middle">
                {lines.map((line, lineIndex) => (
                  <tspan x={centerX} dy={lineIndex === 0 ? 0 : 18} key={`${node.id}-${lineIndex}`}>{line}</tspan>
                ))}
              </text>
            </g>
          );
